...
export function Packages() {
  const t = useT()
  return (
    <section id="packages" className="relative py-24 md:py-32 px-5 md:px-8 border-t border-white/5 bg-[#0F172A]">
            ...
            <RevealItem key={p.key}>
              <div
                className={`relative rounded-2xl p-7 border h-full flex flex-col transition-all hover-lift ${
                  p.highlight
                    ? 'bg-gradient-to-b from-sky-500/10 to-transparent border-sky-500/40 md:-translate-y-3 shadow-[0_20px_60px_rgba(14,165,233,0.18)]'
                    : 'bg-[#1E293B] border-white/8'
                }`}
              >
                {p.highlight && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 chip !bg-sky-500 !text-white !border-sky-400/50 shadow-[0_6px_18px_rgba(14,165,233,0.5)]">
                ...
                <div className="mb-5">
                  ...
                  <span className="text-[12px] text-white/55 ml-2">{p.priceUnit.replace('â‚¬ ', '')}</span>
                </div>

                <div className="text-[11px] uppercase tracking-wider text-sky-400 font-semibold mb-3">
